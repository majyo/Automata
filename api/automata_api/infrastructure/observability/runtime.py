...
from automata_api.infrastructure.observability.redaction import (
    redact_record,
    redact_text,
    sha256_text,
)
from automata_api.infrastructure.observability.retention import enforce_retention
from automata_api.infrastructure.observability.sampler import sample_process_resources
from automata_api.infrastructure.observability.writer import ObservabilityWriter
...
class SpanHandle:
    ...
    def set_status(self, status: str, *, error_type: str | None = None) -> None:
        self.status = status
...
class ObservabilityLogHandler(logging.Handler):
    ...
    def emit(self, record: logging.LogRecord) -> None:
        try:
            ...
            message = redact_text(
                str(record.msg) if isinstance(record.msg, str) else record.name
            )
            self.manager.emit(
                {
                    "record_type": "log",
...
class ObservabilityManager:
    ...
    async def start(self, config: ObservabilityConfig | None = None) -> None:
        if self.started:
            return
        ...
        try:
            ...
            self.normal_queue = queue.Queue(maxsize=self.config.queue_size)
            self.critical_queue = queue.Queue(maxsize=self.config.critical_queue_size)
    ...
    async def stop(self) -> None:
        ...
        self.emit(
            {
                "record_type": (
                    "profile_session_end" if self.profile_enabled else "collector_end"
                )
            },
            critical=True,
        )
    ...
    def emit(self, record: dict[str, Any], *, critical: bool = False) -> None:
        ...
        try:
            ...
            self.increment_stat("critical_enqueued" if critical else "enqueued")
        except queue.Full:
            if critical:
...
def emit_profile_event(name: str, attributes: dict[str, Any] | None = None) -> None:
    manager = get_observability_manager()
