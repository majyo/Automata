...
from automata_api.infrastructure.observability.config import ObservabilityConfig
...
def enforce_profile_retention(profiles_dir: Path, config: ObservabilityConfig) -> None:
    ...
    for directory in profiles_dir.iterdir():
        if not directory.is_dir():
            continue
        target = content if any(directory.glob("content-*.jsonl")) else regular
...
def directory_size(directory: Path) -> int:
    return sum(safe_size(path) for path in directory.rglob("*") if path.is_file())
