...
async def sample_process_resources(
    ...
    *,
    ...
    while manager.started and manager.profile_enabled:
        ...
        managed_processes, live_process_sessions = await managed_process_counts()
        ...
        manager.emit(
            {
                ...
                "attributes": {
                    ...
                    "collector_sample_overhead_ns": (sample_overhead_ns),
                },
            }
        )
...
async def managed_process_counts() -> tuple[int | None, int | None]:
    try:
        managed, sessions = await asyncio.gather(
            get_process_supervisor().active_count(),
            get_process_session_manager().active_count(),
        )
