...
class JsonlRotator:
    ...
    def rotate(self) -> None:
        ...
        while True:
            candidate = (
                self.directory / f"{self.prefix}-{timestamp}-{self.sequence:04d}.jsonl"
            )
            self.sequence += 1
...
class ObservabilityWriter(threading.Thread):
    def __init__(
        ...
        *,
        ...
        self.store = ObservabilityStore(config.output_dir / "observability.db")
    ...
    def profile_dir(self) -> Path | None:
        if self.profile_session_id is None:
            return None
        return self.config.output_dir / "profiles" / self.profile_session_id
