...
def get_observability_config() -> ObservabilityConfig:
    raw_mode = (
        os.environ.get("AUTOMATA_OBSERVABILITY_MODE", "diagnostic").strip().lower()
    )
    ...
    mode: ObservabilityMode = "profile" if raw_mode == "profile" else "diagnostic"
    capture_content = read_bool_env("AUTOMATA_PROFILE_CAPTURE_CONTENT", False)
    ...
    return ObservabilityConfig(
        ...
        queue_size=read_positive_int_env("AUTOMATA_OBSERVABILITY_QUEUE_SIZE", 8192),
        ...
        log_retention_days=read_non_negative_int_env("AUTOMATA_LOG_RETENTION_DAYS", 30),
        ...
        log_level=(
            os.environ.get("AUTOMATA_OBSERVABILITY_LOG_LEVEL", "INFO").strip().upper()
            or "INFO"
        ),
    )
...
def read_positive_int_env(name: str, default: int) -> int:
    ...
    if value <= 0:
        raise ObservabilityConfigurationError(f"{name} must be greater than 0.")
    return value
...
def read_non_negative_int_env(name: str, default: int) -> int:
    value = read_int_env(name, default)
    if value < 0:
        raise ObservabilityConfigurationError(f"{name} must be non-negative.")
    return value
...
def read_int_env(name: str, default: int) -> int:
    ...
    try:
        ...
        raise ObservabilityConfigurationError(f"{name} must be an integer.") from error
