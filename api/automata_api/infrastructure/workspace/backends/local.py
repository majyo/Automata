...
from automata_api.core.tools.results import build_exec_output, exec_command_error_result
from automata_api.core.tools.sandbox import SandboxError
from automata_api.core.tools.text import truncate_content
...
from automata_api.infrastructure.processes.runtime_paths import managed_runtime_roots
from automata_api.infrastructure.processes.shell import (
    resolve_exec_shell,
    shell_argument,
)
from automata_api.infrastructure.processes.tool_sessions import (
...
from automata_api.infrastructure.workspace import paths as workspace_paths
...
class LocalBackend(Backend):
    async def write_stdin(self, arguments: dict[str, Any]) -> ToolResult:
        from automata_api.infrastructure.processes.tool_sessions import run_write_stdin

        return await run_write_stdin(arguments, self.workspace_label)
    ...
    async def stat(self, path: str) -> FileStat:
        ...
        if managed_result is not None:
            return FileStat(
                ...
                path=workspace_paths.path_argument_for_cwd(
                    path_result, self.workspace_path
                ),
    ...
    async def run_exec_command(self, arguments: dict[str, Any]) -> ToolResult:
        ...
        if yield_time_ms is not None:
            try:
                process_session_id = await get_process_session_manager().start(
                    process,
                    timeout_seconds=timeout_seconds,
                    pending_limit=max_output_chars,
                )
                session_output = await get_process_session_manager().interact(
                    process_session_id,
                    chars="",
        ...
        output_truncated = (
            ...
            or output.stdout.truncated
            or output.stderr.truncated
        )
        payload = {
            "simulated": False,
    ...
    async def list_files(
        ...
        *,
        ...
        attempts.append(
            {
                "engine": "filesystem",
                "ok": not filesystem_result.timed_out or bool(filesystem_result.paths),
    ...
    async def _run_path_process(
        ...
        *,
        ...
        accumulator = BoundedPathAccumulator(
            normalize=lambda value: normalize(str(output_base / value)),
            limit=limit,
            max_result_chars=max_result_chars,
        )
        pending = b""
            ...
            async with asyncio.timeout(timeout_seconds):
                while True:
                    if process.stdout is None:
                        break
                    chunk = await process.stdout.read(PROCESS_OUTPUT_CHUNK_BYTES)
                    if not chunk:
                        break
                    pending += chunk
                    ...
                    for raw_path in parts:
                        if not raw_path:
                            continue
                        if accumulator.add(raw_path.decode("utf-8", errors="replace")):
                            terminated_for_limit = True
                            break
                    if terminated_for_limit:
                        break
                if pending and not terminated_for_limit:
                    accumulator.add(pending.decode("utf-8", errors="replace"))
                if terminated_for_limit:
                    await get_process_supervisor().terminate(managed)
    ...
    async def _try_git_file_list(
        ...
        *,
        ...
        if git_executable is None:
            attempts.append({"engine": "git", "ok": False, "error": "not found"})
            return None
    ...
    def _walk_filesystem(
        ...
        *,
        ...
        visit(search_path, 0)
        truncated = accumulator.truncated or (timed_out and bool(accumulator.paths))
    ...
    def _file_list_normalizer(
        ...
        *,
        ...
        def normalize(raw_path: str) -> str | None:
            ...
            try:
                lexical_relative = lexical_path.relative_to(self.workspace_path)
            except ValueError:
                return None
            ...
            if not resolved.is_file():
                return None
            relative_to_workspace = resolved.relative_to(self.workspace_path)
            if not hidden and any(
                part.startswith(".") for part in relative_to_workspace.parts
            ):
                return None
            ...
            if include_globs and not any(
                file_list_glob_matches(normalized, pattern) for pattern in include_globs
            ):
                return None
            if any(
                file_list_glob_matches(normalized, pattern) for pattern in exclude_globs
            ):
                return None
            return normalized
