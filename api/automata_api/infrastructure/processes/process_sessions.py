...
class ProcessSessionManager:
    ...
    async def start(
        ...
        *,
        ...
        if scope is None or scope.session_id is None or scope.workspace is None:
    ...
    async def interact(
        ...
        *,
        ...
        snapshot = ProcessSessionSnapshot(
            ...
            error_code=(sandbox_failure.code if sandbox_failure is not None else None),
            sandbox=(entry.sandbox.to_dict() if entry.sandbox is not None else None),
        )
        if not running:
    ...
    async def terminate_run(self, run_id: str) -> None:
        async with self._lock:
            targets = [
                entry for entry in self._entries.values() if entry.run_id == run_id
            ]
    ...
    async def _authorized_entry(self, session_id: str) -> ProcessSessionEntry:
        ...
        if (
            ...
            and entry.sandbox.profile_hash != scope.permission_profile.profile_hash
        ):
            raise ProcessSessionError(
                "process_session_profile_mismatch",
    ...
    async def _read_stream(
        ...
        pending = entry.stdout_pending if stream == "stdout" else entry.stderr_pending
        pending.append(tail)
    ...
    async def _timeout_session(self, entry: ProcessSessionEntry) -> None:
        try:
            ...
            if entry.process.returncode is None:
                ...
                await get_process_supervisor().terminate(entry.managed)
                entry.activity.set()
    ...
    async def _evict_idle_session(self, entry: ProcessSessionEntry) -> None:
        try:
            while True:
                remaining = PROCESS_SESSION_IDLE_SECONDS - (
    ...
    async def _remove_entry(self, entry: ProcessSessionEntry) -> None:
        ...
        await get_process_supervisor().unregister(entry.managed)
...
def get_process_session_manager() -> ProcessSessionManager:
    from automata_api.core.tools.process_scope import current_process_resources

    resources = current_process_resources()
    return resources[1] if resources is not None else process_session_manager
