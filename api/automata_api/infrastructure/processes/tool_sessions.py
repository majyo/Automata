...
from automata_api.core.tools.text import truncate_head_tail_content
...
async def run_write_stdin(
    ...
    try:
        snapshot = await get_process_session_manager().interact(
            session_id,
            chars=chars_value,
...
def process_session_tool_result(
    *,
    ...
    payload: dict[str, Any] = {
        "simulated": False,
        "ok": snapshot.running or (snapshot.exit_code == 0 and not snapshot.timed_out),
        "tool": name,
