...
from automata_api.infrastructure.extensions.skills.config import (
    SkillsConfig,
    get_skills_config,
)
from automata_api.infrastructure.extensions.skills.loader import (
...
class SkillManager:
    ...
    def skills_for_workspace(
        ...
        key = SkillCacheKey(
            ...
            roots=tuple((str(root.path), root.scope, root.root_id) for root in roots),
        )
        now = time.monotonic()
    ...
    def skill_roots(self, workspace: Path) -> tuple[SkillRoot, ...]:
        ...
        if self._config.packaged_enabled:
            roots.append(SkillRoot(self._config.packaged_root, "packaged", "packaged"))
        roots.extend(
            SkillRoot(
                path,
                "extra",
    ...
    def set_enabled(
        ...
        *,
        ...
        skill = next(
            (
                candidate
                for candidate in outcome.skills
    ...
    def _apply_settings(self, outcome: SkillLoadOutcome) -> SkillLoadOutcome:
        ...
        disabled_paths = frozenset(
            skill.path for skill in outcome.skills if skill.skill_id in disabled_ids
        )
        return replace(
            outcome,
