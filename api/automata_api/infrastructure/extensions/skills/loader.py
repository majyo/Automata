...
def load_skills_from_roots(
    ...
    *,
    ...
    skills.sort(
        key=lambda skill: (scope_rank(skill.scope), skill.name, str(skill.path))
    )
...
def resolve_interface(value: Any, skill_dir: Path) -> SkillInterface | None:
    ...
    interface = SkillInterface(
        ...
        short_description=optional_str(
            value.get("short_description"), MAX_DESCRIPTION_LEN
        ),
        ...
        default_prompt=optional_str(
            value.get("default_prompt"), MAX_DEFAULT_PROMPT_LEN
        ),
    )
    if not any(
        (
