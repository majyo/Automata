...
from automata_api.infrastructure.extensions.mcp.schema import CallPolicy, TrustLevel
...
class McpTrustStore:
    ...
    def save(self, grant: McpServerGrant) -> None:
        grants = {existing.server_fingerprint: existing for existing in self.load()}
        grants[grant.server_fingerprint] = grant
        self._write(tuple(grants.values()))

    def revoke(self, fingerprint: str) -> bool:
        grants = [
            grant for grant in self.load() if grant.server_fingerprint != fingerprint
        ]
