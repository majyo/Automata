...
from automata_api.infrastructure.extensions.mcp.tool import McpAgentTool, mcp_tool_alias
...
class McpToolProvider:
    ...
    def _descriptor_for_tool(
        ...
        metadata = McpToolMetadata(
            ...
            read_only=read_only,
            destructive=(
                False if read_only else bool(annotations.get("destructiveHint", True))
            ),
            idempotent=(
                False if read_only else bool(annotations.get("idempotentHint", False))
            ),
        ...
        exposure = (
            override.exposure
            if override and override.exposure
            else definition.default_exposure
        )
