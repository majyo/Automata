...
from automata_api.core.sessions import rules as session_domain
from automata_api.core.sessions.rules import InvalidBackendError as InvalidBackendError
from automata_api.core.sessions.rules import (
...
from automata_api.core.utils import new_id, normalize_title, now_iso
from automata_api.infrastructure.persistence.db import (
    context_search as context_search_db,
)
...
def create_session(
    ...
    resolved_permission_preset = normalize_session_permission_preset(permission_preset)
    session_id = new_id()
    now = now_iso()
...
def get_messages_after_sequence(session_id: str, sequence: int) -> list[dict[str, Any]]:
    with db_lock, connect_db() as db:
        rows = db.execute(
            """
            SELECT role, kind, content, metadata_json, sequence
