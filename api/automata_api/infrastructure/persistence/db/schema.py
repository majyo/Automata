...
def validate_applied_migrations(applied: dict[int, sqlite3.Row]) -> None:
    ...
    for version, row in applied.items():
        ...
        source = migration_checksum_source(migration)
        # Existing databases were created from both LF and CRLF checkouts.
        checksums = {
            hashlib.sha256(source).hexdigest(),
            hashlib.sha256(source.replace(b"\n", b"\r\n")).hexdigest(),
        }
        if row["name"] != migration.name or row["checksum"] not in checksums:
...
def migration_checksum(migration: Migration) -> str:
    return hashlib.sha256(migration_checksum_source(migration)).hexdigest()


def migration_checksum_source(migration: Migration) -> bytes:
    ...
    for current, historical in migration.checksum_import_aliases:
        source = source.replace(current.encode(), historical.encode())
    return source
...
def assert_database_quick_check(
    ...
    if quick_check is None or str(quick_check[0]).lower() != "ok":
        ...
        raise DatabaseMigrationError(f"Database quick_check failed {stage}: {detail}")
