...
from automata_api.core.agent.prompts import agent_system_prompt
from automata_api.core.agent.types import EventEmitter
...
async def fetch_agent_context(
    *,
    ...
    compressed_messages = build_context_messages(
        system_message, stored_summary, tail_rows
    )
...
def build_context_messages(
    ...
    if summary and summary.get("content"):
        messages.append(
            summary_message(summary["content"], summary["through_sequence"])
        )
...
async def create_context_summary(
    *,
    ...
    response = await provider.complete(summary_request, tools=None)
...
async def create_context_summary_or_none(
    *,
    ...
    except llm.AgentProviderError as error:
