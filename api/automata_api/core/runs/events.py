...
from automata_api.core.runs.event_hub import RunEventHub, run_event_hub
from automata_api.core.runs.ports import RunStore
from automata_api.core.runs.state import RunStateError
from automata_api.core.telemetry import emit_profile_event, emit_record
...
class DurableRunEventSink:
        ...
        emit_record(
            {
                ...
                "attributes": {
                    ...
                    "max_token_buffer_delay_ns": (self._max_token_buffer_delay_ns),
                },
            },
            critical=True,
        )
    ...
    async def _queue_token(self, content: str) -> None:
        async with self._lock:
            ...
            if self._token_timer is None or self._token_timer.done():
                self._token_timer = asyncio.create_task(
                    self._flush_tokens_after_delay()
                )
    ...
    async def _flush_tokens_locked(self) -> None:
        ...
        if timer is not None and timer is not asyncio.current_task():
            timer.cancel()
        await self._persist_and_broadcast_locked({"type": "token", "content": content})

    async def _persist_and_broadcast_locked(self, payload: dict[str, Any]) -> None:
...
def redact_value(key: str, value: Any) -> Any:
    ...
    if isinstance(value, dict):
        return {
            str(child): redact_value(str(child), item) for child, item in value.items()
        }
