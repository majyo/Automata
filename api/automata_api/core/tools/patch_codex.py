...
def parse_codex_hunk(
    ...
    if line == EMPTY_CHANGE_CONTEXT_MARKER:
        ...
        return (
            None,
            start_index,
            f"Expected update hunk to start with '@@' at line {line_number}.",
        )
    ...
    if not hunk_lines:
        return (
            None,
            start_index,
            f"Update hunk does not contain any lines at line {line_number}.",
        )
...
def apply_codex_hunks_to_content(
    ...
    for hunk in hunks:
        ...
        if not old_text:
            return (
                "",
                f"Hunk for {relative_path} must include context or removed lines.",
            )
        ...
        if hunk.context:
            ...
            if context_index == -1:
                return (
                    "",
                    f"Hunk context marker not found for {relative_path}: {hunk.context}",
                )
