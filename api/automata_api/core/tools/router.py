...
from automata_api.core.tools.model import (
    AsyncToolProvider,
    ToolDescriptor,
...
from automata_api.core.tools.providers import BackendToolProvider
from automata_api.core.tools.registry import ToolRegistry
from automata_api.core.tools.tool_search import (
    TOOL_SEARCH_NAME,
    run_tool_search,
    tool_search_spec,
)
...
class ToolRouter:
    ...
    def model_visible_specs(self, *, mode: str = "act") -> list[dict[str, Any]]:
        ...
        if len(specs) < self._max_model_tools and self._search_candidates(mode=mode):
            specs.append(tool_search_spec())
        return specs

    def allowed_names(self, *, mode: str = "act") -> set[str]:
        names = {descriptor.name for descriptor in self._visible_descriptors(mode=mode)}
        if len(names) < self._max_model_tools and self._search_candidates(mode=mode):
    ...
    async def dispatch_authorized(
        ...
        *,
        ...
        if not self._is_model_visible(descriptor, mode=mode):
            return await self.dispatch(name, raw_arguments, mode=mode)
        return await self._registry.run_authorized(name, raw_arguments, mode=mode)
    ...
    def _install_descriptors(self, descriptors: tuple[ToolDescriptor, ...]) -> None:
        self._descriptors = descriptors
        self._descriptors_by_name = build_descriptor_index(descriptors)
        self._registry = ToolRegistry(descriptor.executor for descriptor in descriptors)
        self._activated_deferred = {
            name: None
            for name in self._activated_deferred
            if name in self._descriptors_by_name
...
def build_descriptor_index(
    ...
    for descriptor in descriptors:
        if not isinstance(descriptor.name, str) or not descriptor.name.strip():
            raise ValueError(
                f"Registered tool name must be a non-empty string: {descriptor!r}"
            )
