...
from automata_api.core.tools.constants import SEARCH_TIMEOUT_SECONDS
from automata_api.core.tools.models import ToolResult
from automata_api.core.tools.results import search_error_result, search_tool_result
from automata_api.core.tools.workspace import Backend, BackendError
    ...
    return {
        ...
        "properties": {
            ...
            "timeout_seconds": {
                "type": "number",
                "description": (
                    "Optional timeout in seconds. Defaults to 30 and is capped at 120."
                ),
            },
        },
        "required": ["pattern"],
    ...
    properties.update(
        {
            ...
            "include_globs": {
                ...
                "description": ("Files mode only. Include paths matching any glob."),
            },
            "exclude_globs": {
                "type": "array",
                "items": {"type": "string"},
                "maxItems": MAX_FILE_LIST_GLOBS,
                "description": ("Files mode only. Exclude paths matching any glob."),
            },
            "hidden": {
                "type": "boolean",
                "description": (
            ...
            "max_depth": {
                ...
                "description": ("Files mode only. Traversal depth relative to path."),
            },
            "limit": {
                "type": "integer",
                "minimum": 1,
...
async def run_file_list(
    ...
    if result.truncated:
        payload["hint"] = "Narrow path or include_globs and call rg again."
...
def parse_globs(
    ...
    for item in value:
        ...
        if len(item) > MAX_FILE_LIST_GLOB_CHARS:
            return (), (
                f"{name} entries must be at most {MAX_FILE_LIST_GLOB_CHARS} characters."
            )
        if "\x00" in item:
