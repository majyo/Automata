...
from automata_api.core.agent.skills.diagnostics import (
...
from automata_api.core.tools.management import SkillCatalog
from automata_api.core.tools.router import ToolRouter
from automata_api.transport.dependencies import skill_catalog
from automata_api.transport.schemas import (
...
async def list_skills(
    ...
    catalog: SkillCatalog = Depends(skill_catalog),
) -> SkillsListResponse:
    normalized_workspace = normalize_workspace(workspace)
    outcome = catalog.skills_for_workspace(
        normalized_workspace,
        force_reload=force_reload,
    )
    diagnostic_router = catalog.router(normalized_workspace)
...
async def set_skill_enabled(
    ...
    return skill_record(
        ...
        workspace=workspace,
        diagnostic_router=catalog.router(workspace),
        catalog=catalog,
    )
...
async def get_skill_diagnostics(
    ...
    normalized_workspace = normalize_workspace(workspace)
    outcome = catalog.skills_for_workspace(normalized_workspace)
    skill = next(
    ...
    diagnostics = diagnose_skill_dependencies(
        skill,
        router=catalog.router(normalized_workspace),
        workspace=normalized_workspace,
        mcp_lookup=catalog.mcp_lookup,
    )
